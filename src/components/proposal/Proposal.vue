	...
	<div>
		...
		<mu-flexbox orient=vertical class="content">
			...
			<mu-flexbox class="proposl proposal">
				...
				<mu-raised-button class="demo-raised-button" label="保存为经验方" />
			</mu-flexbox>
			<mu-divider/>
		</mu-flexbox>
		<!-- 药品容器 -->
		...
		<!-- <mu-paper class="bottomnav">
			<mu-bottom-nav :value="bottomNav" shift @change="handleChange">
				...
				<mu-bottom-nav-item value="alarm" title="Books" icon="alarm" />
...
export default {
	...
	watch: {
		//用侦听器监听input值的变化
		// diseaseName(curVal, oldVal) {
		// 	//如果保存的最终药品不为空时 为diseaseNames重新赋值，即最终要渲染的药品列表
		// 	if (this.diseaseNames) {
		// 		this.diseaseNames = curVal;
		// 	} else {
		// 		//保存的最终药品为空时，为第一次输入，要用空覆盖掉用户输入的值
		// 		this.diseaseNames = '';
		// 	}
